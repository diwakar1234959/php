...
?>
    <form class="form" action="login_status.php" method="POST">
        <h1 class="login-title">Login Status</h1>
        <input type="hidden" name="id" value="<?php echo $row['id']; ?>"> <!-- Hidden input for user ID -->
        <!-- <input type="text" class="login-input" name="package" placeholder="Package"value="<?php echo $row['login']; ?>" /><br><br> -->
        <label for="login">login:</label>
        <select name="login">
            <!-- <option>Select Your Option</label> -->
           <?php if($row['login']=="1"){ ?>
            <option value="<?php echo $row['login']; ?>">inactive</option>
            <?php } ?>
            <?php if($row['login']=="0"){ ?>
            <option value="<?php echo $row['login']; ?>">active</option>
            <?php } ?>
    </select><br><br>
